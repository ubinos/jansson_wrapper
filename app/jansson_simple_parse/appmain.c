...
void print_json_array(json_t *element, int indent) {
    ...
    printf("JSON Array of %d element%s:\n", size, json_plural(size));
    for (i = 0; i < size; i++) {
...
static char _line[MAX_CHARS];

int simple_parse_main(int argc, char *argv[]) {
    ...
    while (read_line(_line, MAX_CHARS) != (char *)NULL) {

        /* parse text into JSON structure */
        json_t *root = load_json(_line);
...
static void root_func(void * arg)
{
    ...
    printf("\n\n\n");
    printf("================================================================================\n");
    printf("jansson_simple_parse (build time: %s %s, logm category: %d)\n", __TIME__, __DATE__, LOGM_CATEGORY);
    printf("================================================================================\n");
    printf("\n");

    simple_parse_main(0, NULL);
}
